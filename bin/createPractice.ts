...
const setDefaultLocalStorage = async (page: Page) => {
  await page.evaluate(() => {
    // @ts-ignore
    localStorage.setItem('global_lang_key', '"rust"')
    // @ts-ignore
...
let tagIndex = 0
let classification: string = (tags.length > 0 ? tags[tagIndex] : '未知') as string
...
while (tagIndex < tags.length && classificationStr === 'other') {
  classification = tags[++tagIndex] as string
...
const filePath = join(__dirname, '../src/', classificationStr, fileName + '.rs')
const imageFilePath = join(__dirname, `../images/${classificationStr}`, fileName + '.jpeg')
const testFilePath = join(__dirname, '../tests/', classificationStr, fileName + '_test.rs')
...
if (readmeFileContent.includes(url)) {
  ...
  // * 不要删除下面存在的空行
  const instructions = `
- [${title}](src/${classificationStr}/${fileName + '.rs'})  [${tags.join(', ')}]
